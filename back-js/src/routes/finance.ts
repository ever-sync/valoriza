...
export async function financeRoutes(app: FastifyInstance) {
  ...
  app.post('/contrato/:id/lancar-parcelas', { preHandler: authenticate }, async (request) => {
    ...
    const total = Number(contrato.valor_solicitado); const qtd = Number(contrato.quantidade_parcelas); const taxa = Number(contrato.taxa_juros) / 100
    const parcela = contrato.modelo_amortizacao?.toLowerCase() === 'sac' ? total / qtd : taxa ? total * (taxa * (1 + taxa) ** qtd) / ((1 + taxa) ** qtd - 1) : total / qtd
  ...
  app.post('/contrato/simular', { preHandler: authenticate }, async (request) => {
    const body = request.body as Record<string, unknown>; const total = money(body.valor_solicitado); const qtd = Number(body.quantidade_parcelas); const taxa = money(body.taxa_juros) / 100
    const parcela = taxa ? total * (taxa * (1 + taxa) ** qtd) / ((1 + taxa) ** qtd - 1) : total / qtd
